...
const JobApplicationForm = () => {
  ...
  // Allow submit when basic fields are ready, regardless of jobInfo state
  const isValidEmail = (email) => /.+@.+\..+/.test(email);
  const canSubmit = Boolean(
    formData.fullName && isValidEmail(formData.email) && pdfText
  );

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!canSubmit) {
      alert('Please provide your name, a valid email, and upload your resume (PDF).');
      return;
    }
    setLoader(true);
    try {
      ...
      // Try parse JSON; if not JSON, still continue to success UX
      let data = null;
