    ...
    <div className="bg-white rounded-lg shadow-xl mb-6">
      ...
      <div className="border-b p-6 flex justify-between items-center">
        ...
        <div className="text-sm text-gray-500">
          Showing {safeJobs.length === 0 ? 0 : indexOfFirstJob + 1}-{Math.min(indexOfLastJob, safeJobs.length)} of {safeJobs.length} jobs
        </div>
      </div>
  ...
  useEffect(() => {
    const fetchCandidates = async () => {
      try {
        ...
        const candidateList = Array.isArray(response.data.candidateData)
          ? response.data.candidateData
          : [];

        const aptitudePass = Array.isArray(response.data.aptitudePassesCandidates)
          ? response.data.aptitudePassesCandidates
          : [];
        const aptitudeFail = Array.isArray(response.data.aptitudeFailedCandidates)
          ? response.data.aptitudeFailedCandidates
          : [];
        const techPass = Array.isArray(response.data.techPassesCandidates)
          ? response.data.techPassesCandidates
          : [];
        const techFail = Array.isArray(response.data.techFailedCandidates)
          ? response.data.techFailedCandidates
          : [];

        const enrichedCandidates = candidateList.map(
          (candidate) => ({
            ...candidate,
            aptitudeStatus: aptitudePass.includes(
              candidate.email
            )
              ? "Passed"
              : aptitudeFail.includes(candidate.email)
                ? "Failed"
                : "Pending",
            techStatus: techPass.includes(
              candidate.email
            )
              ? "Passed"
              : techFail.includes(candidate.email)
                ? "Failed"
                : "Pending",
            hrStatus: "Pending",
    ...
    const fetchScores = async () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          setCandidateScores(data.reduce((acc, scoreData) => {
        ...
        // If error 404 or no data, just show empty (not an error to the UI)
        setCandidateScores({});
      } finally {
