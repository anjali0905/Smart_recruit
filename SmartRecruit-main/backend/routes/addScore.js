...
router.post('/addScore', async (req, res) => {
  try {
    ...
    // Validate roundName
    const validRounds = ['technical', 'aptitude', 'communication', 'hr'];
    if (!validRounds.includes(roundName.toLowerCase())) {
      return res.status(400).json({
        success: false,
        message: "Invalid round name. Must be technical, aptitude, communication, or hr"
      });
    }
    ...
    if (!candidateScore) {
      candidateScore = new CandidateScore({
        ...
        hrInterviewScore: roundName.toLowerCase() === 'hr' ? score : 0
      });
    } else {
      ...
      switch (roundName.toLowerCase()) {
        ...
        case 'technical':
          ...
          break;
        case 'hr':
          candidateScore.hrInterviewScore = score;
          break;
      }
    }
